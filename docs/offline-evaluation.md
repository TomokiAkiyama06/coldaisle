...
**指定した期間に decision trace が1つも無い run も拒みます。** 黙って通すと、その run は
segment も gate も作らないので報告のどこにも現れず、**ほかの run だけで `pass` が出ます**
（0054 §2.5 の「tick の無い segment を作らない」と同じ理由）。
...
`SqliteStore` は**開くだけで** WAL を設定し、未適用の migration を当て、path が無ければ
DB を作ります。証拠として読む DB をそれで開くと、**古い run の DB を評価した瞬間に
スキーマが上がり、評価が証拠を書き換えて**しまいます（0054 §2.7 の「読み取りのみ」）。

`coldaisle-evaluate` は sqlite の `mode=ro` で開き、**版を確かめるだけで何も変えません。**

- path が無ければ**作らずに**落ちる（空の DB を作って「証拠が無い」にしない）
- スキーマが必要な版より古ければ落ちる（**移行はしない**。必要なら別の手段で移行する）
- スキーマがこのコードより新しくても落ちる（読み違えない）
- 必要な版は `migrations` の並びから引く（数字を写さない）
