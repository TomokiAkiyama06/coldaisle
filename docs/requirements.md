...
1. サーバー未着のため、実機に依存する開発が進められない。（2026-08-23 時点の課題。サーバーは 2026-09-13 時点で導入済み。実機が必要な Issue は `requires:server` / `requires:sensor-module` のラベルで区別する）
...
**FR-504の理由**: ローカルモデルは長大な数値テーブルに対する算術・比較が不安定です。
...
**Qwen3.8-27B** を採用します（dense 27B、Apache 2.0）。

当初は小型モデルと大型モデルの二層構成を想定していましたが、**単体構成に変更します。**
...
**大型モデルを併置する前に、単体で不足するかを実測する**という順序を取ります。

不足が確認された時点で、上位モデルの追加を検討します。
Provider 抽象（Issue #21）があるため、差し替えは設定変更で済みます。

> **要検証**: vLLM が Blackwell (sm_120) でこのモデルを動かせること。
> Qwen3.5系以降は Gated DeltaNet を使うため、ランタイムのバージョン要件があります。
> 導入時に必ずスモークテストを通してください。
> HuggingFace のリポジトリ ID（`Qwen/Qwen3.8-27B` を想定）も導入時に実物で確認してください。

> 決定記録: [`decisions/0005-model-selection.md`](decisions/0005-model-selection.md)
> （0001 D-13 の二層構成を置き換え）

### 7.2 実行環境（フェーズ別）

| フェーズ | ホスト | ランタイム | 形式 |
|---|---|---|---|
| 現在（サーバー未着） | Mac | **Ollama** または LM Studio / MLX | Q4_K_M 量子化（27B で約17GB） |
| 本番 | Ubuntu + RTX PRO 6000 | **vLLM** | BF16 または FP8 |

> Mac 側は搭載メモリ次第です。27B の Q4_K_M が乗らない場合は、
> **開発中の代替として小型モデルを使ってかまいません。** Provider 抽象（FR-501）が
> 差し替えを吸収するため、本番の選定（7.1）とは独立に決められます。
...
`--gpu-memory-utilization 0.4` で上限を切ります（重み 30〜33GB に KV プールを足した値）。
ただし **v0.2 での重要な変更点** として、これは「常駐させてよい」という意味ではありません。
...
| LLM | Qwen3.8-27B（Ollama → vLLM） | OpenAI互換API |
| Lint/Format | ruff | |
| 型 | mypy（strict） | |
| テスト | pytest | |
...
| Q-08 | ローカルモデルの役割分担 | ~~Qwen3-8B を主軸 + GPT-OSS-120B を併用~~ → **Qwen3.8-27B 単体**（決定記録 0005 で改訂） | 2026-08-24 |
...
| Q-22 | MPC / Supervisor の周期、MPC 目的関数の重み | 方式（内部モデル要件・探索・Hard Constraints）は決定記録 0052（FINAL、2026-09-20 所有者承認）で決定。**horizon / step / 周期 / 重み・基準量の値**は実測後に GitHub #86 / #88 / #103 |
...
| Q-26 | 音響センサーのハードウェア | 測定方式は決定記録 0067（Proposed）と `docs/acoustic-measurement.md`。**機材の選定**は所有者が GitHub #95 で決める |
