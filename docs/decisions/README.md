...
| [0067](0067-acoustic-measurement-study.md) | 騒音は概算で測る（専用機材なし・曲線は `approximate` のまま・測定を制御ループの外に置く・Fan は `CALIBRATION` だけで動かす・録音しない・周波数情報は耳で補う・普段の運転中の「うるさいか」の回答で不快の下限を探す案） | Proposed |
