...
- **関連**: [`0027-fan-control-architecture.md`](0027-fan-control-architecture.md) /
  ...
  `docs/requirements.md` S-13 / D-03 /
...
- Fan 制御は専用のデーモン（以下 `coldaisle-fand`）で行う。取り込みデーモン・API・GPU AI Service とは別プロセスにする。
  **hwmon の PWM へ書き込めるのはこのプロセスだけ**とする
...
- Learned MPC / RL Supervisor（M9）は**制御ループの外の worker プロセス**で動かす。
  worker は提案を時刻付きで置き、ループは最新の提案を読むだけにする。worker が落ちても、止まっても、ループは止まらない。
  Fallback Controller と RulePolicy は決定論的で軽いので、ループの中で動かす
- Telemetry の入り口は2つに分ける
  - CPU / GPU / Fan（hwmon / NVML。#65）：制御プロセスが**毎 tick 直接読む**。Top の安全判定を取り込みデーモンに依存させないため
  - 外付けセンサーの `air.*`：**ストアの `latest()` を読み取り専用で読む**（SQLite WAL。決定記録 0004）。
    シリアルを開くのは取り込みデーモンだけ（AGENTS.md ルール6）。取り込みデーモンが止まれば `air.*` が古くなり、2.7 の扱いになる
...
各層の出力は **`tick_id` と `ts_ms` を共有**し、1 tick の判断の記録（decision trace）として #82 が保存する。型の定義は #76 が実装する。

| 層 | 入力 | 出力 | 有効期限 |
|---|---|---|---|
| Telemetry snapshot | hwmon / NVML / `latest()` | metric ごとに `value` / `ts_ms` / `age_ms` / `status`（`fresh` / `stale` / `missing`） | その tick だけ |
| State Estimator | snapshot | 派生量（`dT/dt`、Power slope、case ΔT など）、`regime` と `regime_confidence` | その tick だけ |
| Supervisor | state | `policy` / `regime` / `weights` / `strategy` / `version` / `computed_at_ms` / `valid_until_ms`。**demand は出さない** | `valid_until_ms` まで |
| 制御器（Fallback / Learned MPC） | state、Supervisor の出力（任意） | zone ごとの `requested_demand` と `reason`。全体で `controller` / `model_version` / `confidence` / `ood` / `optimizer_status` / `latency_ms` / `computed_at_ms` / `valid_until_ms` | `valid_until_ms` まで（Fallback はその tick だけ） |
...
| Reactive Guard | state、`requested` | zone ごとの `floor` / `ceiling` / `hold_until_ms` / `reason` | `hold_until_ms` まで |
| Critical Safety | snapshot、Hardware の状態、`requested`、Guard の出力 | zone ごとの `floor` / `forced_max` / `faults` / `reason`、全体の `safety_state` | その tick だけ |
...
x1 = min(r,  guard_ceiling)                 Guard の ceiling（無ければ 1.0）
...
effective = 1.0 if forced_max else x4       startup / emergency / zone の fault
```
...
| 出すもの | 一時的な floor / ceiling と hold | 最低安全 demand、CPU cooling floor、下げる速さの制限、forced Max |
...
| モード | requested の出どころ |
|---|---|
| `AUTO`（既定） | Gate が選んだ制御器 |
| `MANUAL` | 人が zone ごとに指定した値 |
| `MAX` | すべて 1.0 |
| `CALIBRATION` | 人が開始した測定計画（#75）の値 |

- **どのモードでも Reactive Guard と Critical Safety を通る。** `CALIBRATION` でも外さない。Guard が介入した時点は記録し、その測定区間に印を付ける
...
- モードが `AUTO`
- stage が `LIMITED` 以上
- 提案が有効期限内（`now_ms ≤ valid_until_ms`）で、`optimizer_status` が正常
...
時刻は `Clock`（`src/coldaisle/clock.py`、#73）だけから取る。試験では `SimulatedClock` で進める。値はすべて設定に置き、下の数値は**初期の暫定値**とする。
...
| `mpc.period_ms` / `mpc.budget_ms` / `mpc.valid_ms` | 10000 / 2000 / 20000 | ML の提案周期・計算の予算・提案の有効期限。予算を超えた結果は捨て、`optimizer_status = timeout` として Fallback を使う |
...
- tick は**絶対時刻の締め切りで刻む**（ドリフトしない。決定記録 0022 と同じ考え方）。遅れた tick を後から取り戻して連続実行しない
...
| 事象 | 対応 | safety state |
|---|---|---|
| CPU 温度が取れない / stale | Top を Max | `DEGRADED` |
| GPU（NVML）が取れない / stale | Front / Rear を `fault_demand` | `DEGRADED` |
| T_SENSOR が取れない / stale | Front / Rear を `fault_demand` | `DEGRADED` |
...
- **制御を取るとき**：`pwmN_enable` を手動へ切り替える**前に**、元の `pwmN_enable` と `pwmN` の値、対象のファイルの場所を実行時ファイル（`/run` の下。リポジトリには置かない）へ保存する。その後に `STARTUP` の Max を書く
- **正常停止**（SIGTERM で終了し、systemd の `SERVICE_RESULT` が `success`）：保存した元の `pwmN_enable` へ戻し、**BIOS の制御へ返す**
- **異常終了**（例外・SIGKILL・watchdog）：systemd の `ExecStopPost` で**引き継ぎ用の小さな補助プログラム**を動かし、保存した対象へ Max を書く。
  補助プログラムは coldaisle の Python パッケージ・設定・モデルに依存させない（壊れた環境でも動くようにする）。実行時ファイルが無ければ、制御を取っていないので何もしない
- **再起動**：`Restart=always`。起動するたびに `STARTUP`（Max）を通る
- **残るリスク**：OS ごと止まるなど `ExecStopPost` が動かない場合、PWM は最後の値のまま残る。制御を取った後は BIOS Q-Fan も自動では戻らないため、このときの最終防衛線は **CPU / GPU 自身のサーマルスロットリング**になる（要件 D-03 の Safety-0 のうち、制御を取った後も残るもの）。
...
| 3 | **実機のファンの制御を初めて取る**（Hardware Backend の書き込みを有効にする） | #74 の実機確認 | 所有者が現地で行う。kill・watchdog・正常停止時の引き継ぎを確認項目にする |
...
| 引き継ぎの補助プログラムという部品が増える | 依存を持たない小さな補助プログラムにし、偽の sysfs で試験する。実機確認は承認点 3 |
...
| `air.*` をストア経由で読むため、取り込みデーモンの停止が Front / Rear に波及する | 波及先は安全側（`fault_demand`）。遅延が問題になれば未決 8 で見直す |
...
| 正常停止でも Max にする | 人が明示的に止めたときは、BIOS の制御（導入前の既知の状態）へ返す。Max は異常終了に使う（2.7） |
...
| 8 | `air.*` をストア経由で読む遅延が許容できるか。できなければ取り込みデーモンからのローカルソケットにする | #74（実測） |
