...
- **照合の許容幅は `fan-policy.yaml` の `shadow` から取り、記録された幅と照らす**（0054 §2.6）。
  outcome どうしが揃っているだけでは足りない。全部が揃って「別の幅」で照合されていれば、
  記録された coverage と意味の違う証拠がそのまま drift の根拠になる
- **上の規則は検知器の境界で閉じる。** 入口（CLI）だけで閉じると、同じ型を library 経由で
