...
- **Date**: 2026-09-20
- **Superseded by**: [0049](0049-internal-telemetry-source-kind.md)（(1) §2.1 のうち、CPU 使用率の
  **いまの値**の札を「内部テレメトリの『読み取り値』」に固定する部分、(2) §2.3 のうち、グラフの
  過去の値を「実測値」と言い切る文言のみ。いまの値の札は `sys.telemetry_kind` と鮮度から
  実測 / 模擬 / 読み取り値 になり、過去の値の出どころは表示しない。「未計測 / 未取得」の
  判断（§2.2）、§2.3 の規則そのもの（無効化後も過去の行を描く）、その他の節は有効）
