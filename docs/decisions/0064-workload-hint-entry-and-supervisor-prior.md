...
- **Date**: 2026-09-21
- **Superseded by**: [0066](0066-workload-hint-stage-b-conditions.md)（§2.8 の「単調性（片方向）」の検証方法、§2.6 の「起動時の取り込み」の条件式、§2.4 の「スキーマ変更は要らない」の記述、§2.1 の「実装の変更点は `messages.py` の予約解除と型の追加に限る」という範囲の制約のみ。他の節は有効）
