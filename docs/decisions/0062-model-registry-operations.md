...
制御モデルの昇格・rollback という安全側の運用手順を決めるため、実装担当モデルに関係なく
人間のレビューを必須とした（AGENTS.md「実装の担当」）。所有者が 2026-09-21 に、次の4点を
名指しで承認した。

1. 承認をファイルで渡す運用（§2.2）
2. `verify` の終了コードを 0 / 3 / 4 に分けること（§2.4）
3. registry の lifecycle event を decision trace へ載せるかを #82 側の決定に委ねること（§2.5）
4. YAML の入れ子の深さを自前の走査ではなく parser に測らせること（§2.1）

**§5 の未決事項と、設定の暫定値は開いたままである。** 承認したのは上の判断であって、
そこで挙げた未決事項が閉じたわけではない。
