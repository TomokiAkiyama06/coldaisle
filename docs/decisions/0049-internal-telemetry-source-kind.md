# 決定記録 0049: 内部テレメトリの出どころの種類（実機 / 模擬）を記録し、**いまの値**に「実測」と書く

- **種別**: Decision Record
- **Status**: FINAL（2026-09-20、リポジトリ所有者が承認）
- **Date**: 2026-09-20
- **Supersedes**（いずれも値の出どころの言い方だけを、範囲を限って置き換える。§2.5）:
  - [0046](0046-airflow-ui.md) §2.5 の表の「PWM」の行のうち、札を `PWM（実測） 48%` と
    固定する部分のみ。札の語は出どころと鮮度で決まる（実測 / 模擬 / 読み取り値）。
    同じ行の他の内容（保存値は 0〜100 %、Demand ではなく実測の PWM）と、0046 の他の節
    （§2.7 の取り込み経路 `air.*` の見出しの表示を含む）は有効
  - [0051](0051-airflow-cpu-utilization-display.md) §2.1 のうち、CPU 使用率の**いまの値**の
    札を「内部テレメトリの『読み取り値』」に固定する部分のみ。0051 の「未計測 / 未取得」の
    判断（`cpu_utilization.measured`）とその他の節は有効
  - [0051](0051-airflow-cpu-utilization-display.md) §2.3 のうち、グラフの過去の値を
    **「実測値」と言い切る文言のみ**。模擬の adapter（`source_kind: mock`）で書いた区間の
    履歴は実測ではないため、本記録では過去の値の出どころを主張しない（§2.5）。
    同じ節の規則（収集を無効にしたあとも、過去に保存された行はグラフに描く）は有効

  他の記録の文言は変えない。0004 §2.2（`stale` の付け方）・0042 §2.4（届いている値の
  数え方）・0044（JS テスト）・0046 §2.7 は**そのまま使う**（本記録はそれらを参照するだけ）。
  0046 §2.5 の「Demand ではなく実測の PWM」「回転数は実測できている」（AIO ポンプの行）と
  §3 の「実測できている回転数・PWM…」は、
  **どの量か**（Demand ではなく PWM の読み戻し）・実機運用を前提にした説明であって、
  値の出どころの札の決め方ではない。札は本記録が定める。
- **関連**: [`0002-metric-naming.md`](0002-metric-naming.md) §2.6 /
  [`0009-read-api.md`](0009-read-api.md) §2.8 /
  [`0040-server-health-api.md`](0040-server-health-api.md) /
  [`0046-airflow-ui.md`](0046-airflow-ui.md) §2.7 /
  [`0047-cpu-utilization-metric.md`](0047-cpu-utilization-metric.md) /
  [`0051-airflow-cpu-utilization-display.md`](0051-airflow-cpu-utilization-display.md) §2.1 /
  #106 / PR #140 / PR #146
- **対象 Issue**: #147

## 1. Context

エアフロー画面（0046）は、値の札と注記を**経路ごとに**書き分けている。

| 経路 | 書くメトリクス | 出どころの手がかり | 画面の表示 |
|---|---|---|---|
| 取り込み（`coldaisle-daemon`） | `air.*`（`channels.py` の `CHANNEL_TO_METRIC`） | `sys.ingest_source` → `/api/v1/health` の `source`（serial / mock / replay） | 実測 / 模擬 / 再生 / 出どころ不明 |
| 内部テレメトリ（`coldaisle-telemetry`） | `fan.*.rpm` / `fan.*.pwm` / `cpu.*` / `gpu.*` など | **無い** | 中立な「読み取り値」 |

2026-09-19 時点のコード（origin/main）で確認したこと:

1. **内部テレメトリのメトリクスを書くのは `coldaisle-telemetry` だけ。** 取り込みの
   MockSource / ReplaySource は `SAMPLE_CHANNELS`（空気の温度・湿度）しか書かず、
   `control/hardware/simulated.py` も DB へ書かない。
2. `coldaisle-telemetry` の CLI には `--source` が無く、`build()` は既定で実 adapter
   （`NvmlAdapter` / `HwmonAdapter` / `ProcStatAdapter`）を組む。模擬の adapter は
   `build(adapters=...)` で差し込む試験用の経路にしか無い。
3. ただし「実 adapter のクラスである」ことは「実機を読んでいる」ことを保証しない。
   `NvmlAdapter(config, api=...)` は NVML の呼び出しを差し替えられ、`hwmon.root` と
   `proc_stat.path` は設定で任意の path を指せる（試験は偽の sysfs を使う）。
4. `sys.telemetry_source.<adapter>` は adapter ごとの**稼働状態**（ok / degraded /
   unavailable / disabled）で、0040 の Server Health（`sources.nvml` / `sources.lm_sensors`）が
   読む。出どころの種類は表さない。
5. `/api/v1/health` の `source` は `sys.ingest_source` だけを返す。画面（`airflow.js`）は
   この値を `air.*` の札にだけ使い、内部テレメトリは `TELEMETRY_KIND = "読み取り値"` 固定。

このため、実機で動かしていても回転数・PWM・CPU・GPU を「実測」と書けない。
逆に、根拠なく「実測」と書くと、模擬の DB を見ているときに誤解させる（0046 §2.7 の
「分からないときに実機と言わない」に反する）。**先にデーモンが出どころを記録する必要がある。**

2026-09-20、リポジトリ所有者が**いまの値だけに札を付ける単純な案**を選んだ。
過去の値（グラフ・履歴）の出どころは画面に出さない。表示の注記のために過去の区間まで
判定する仕組み（§4 の案 Q）は、得るものに対して作るものが大きいと判断した。

## 2. Decision

### 2.1 出どころの種類は、テレメトリデーモンを組む側（合成の起点）が決める

種類を決めるのは `telemetry_daemon.build()`（合成の起点）とする。adapter 自身や
設定ファイルには申告させない（§4 の案 C / D）。

- `adapters` を渡さない（CLI からの通常の起動）→ **`hardware`**
- `adapters` を渡す → 呼び出し側が種類を**必ず明示する**（引数 `source_kind`）。
  **既定値を持たせない。** 既定を `hardware` にすると、試験の偽 adapter が黙って
  「実機」を名乗る

`hardware` の意味は「実 adapter が、設定された OS の読み取り口（NVML / hwmon / `/proc/stat`）を
読んでいる」である。**センサーと物理部位の対応が正しいこと**までは表さない
（それは hwmon の `confirmation`（0029 §2.4）が担う）。

### 2.2 取りうる値

| 値 | 意味 |
|---|---|
| `hardware` | 実 adapter による読み取り（§2.1） |
| `mock` | 差し込まれた模擬の adapter（試験・将来の模擬デーモン） |

- 値は `internal_telemetry/models.py` の `StrEnum`（例: `TelemetrySourceKind`）に置く。
  運用で変える値ではなく実装の識別なので、設定ファイルには出さない（ルール9 の対象外）
- **`replay` は今は定義しない。** 内部テレメトリを再生する経路が存在しないため。
  作るときに別の決定記録で足す
- 「不明」は値として保存しない。**キーが無い／知らない値**を不明として扱う（§2.5）

### 2.3 記録先: `system_state` の `sys.telemetry_kind`

- キーは **`sys.telemetry_kind`**（値は §2.2）。デーモン全体で1つ
- `run()` の最初の収集の前に、収集と同じ `Clock`（Unix ms）の時刻で書く
- `set_system_state` の規則（0002 §2.6）どおり**変化時だけ**書く。`sys.ingest_source`
  （0009 §2.8）と同じ形で、書き込みの特別扱いを足さない
- `sys.telemetry_source.` の下には置かない。その prefix は adapter 名ごとの稼働状態の
  名前空間で、0040 の Server Health が読んでいる。混ぜると「adapter 名 = kind」と区別が付かない
- キー名は `internal_telemetry/models.py` の定数にし、API はそれを import する
  （`server_health.py` が `SOURCE_STATE_PREFIX` を import しているのと同じ向き。
  L2 → 下位の一方向で、逆向きの import は生じない）
- **スキーマの変更は無い**（既存の `system_state` を使う）
- 1つのデーモンの adapter は同じ合成で組まれるので、種類はデーモン全体で1つに決まる。
  adapter ごとに分ける（§4 の案 B）のは、実機と模擬を混ぜる合成が必要になってからにする

**変化時だけ書く結果、DB には時刻付きの履歴が残る。** 本記録はその履歴を読まないが、
将来「過去の値にも出どころを出す」と決めたときに、書き込み側を作り直さずに広げられる（§3）。

### 2.4 API: `/api/v1/health` に**いまの種類**を1つ足す

- `HealthResponse` に `telemetry_source: str | None` を足す。`sys.telemetry_kind` の
  現在値をそのまま返す（行が無ければ `null`）。既存の `source`（取り込み）は意味を変えない
- **新しい endpoint は作らない。** 画面はすでに `/api/v1/health` を読んでいる（`airflow.js`）。
  いまの種類は値1つなので、既存の応答に足すのがいちばん小さい
- `/api/v1/latest` / `/api/v1/series` の応答は変えない
- **Server Health（`/api/v1/server-health`、0040 / 0042）の契約も変えない。** あちらは
  稼働状態の契約。必要になれば別の記録で足す

### 2.5 画面: **いまの値**にだけ札を付ける。過去の値の出どころは出さない

**札が付くのは「いま届いている値」だけ。** 値ごとに `/api/v1/latest` を見て、
**`value` が `null` でなく**、かつ `quality` が `ok` または `suspect` のときだけ種類の札を付ける。
`value` が `null`・`stale` / `missing`・キーが無い値は**中立な「読み取り値」**のままにする。

- **鮮度の規則は既存のものを使う**（新しい定数を作らない）。`latest()` は
  `config/quality.yaml` の `stale_after_ms` 以上古い値を `stale` に落とす（0004 §2.2）。
  「`ok` / `suspect` は届いている、`stale` / `missing` / 未保存は届いていない」という切り方も、
  Server Health の規則（0042 §2.4）と同じにする
- `/latest` は値ごとに `value` / `quality` / `age_seconds` を持つ。画面は `value` と `quality` で
  判定する（`age_seconds` は表示用。しきい値を画面側で持たないため）
- **`quality` だけでは足りない。** `HwmonAdapter._read_sensor()` は有限でない読み値を
  `value: null` / `quality: suspect` で保存し、`/latest` は新しいうちそのまま返す。画面は
  値が無いので「未取得」と出す（`airflow.js` の `reading()`）。ここに出どころの札を付けると、
  表示の無い値に「実測」が付く

| 表示 | 札 |
|---|---|
| いまの値のうち **届いている**もの（`value` が非 `null` かつ `quality` が `ok` / `suspect`） | `hardware` → **実測** / `mock` → **模擬** / `null`・知らない値 → **読み取り値** |
| いまの値のうち **届いていない**もの（`value` が `null`、`stale` / `missing`、キーが無い） | **読み取り値**（今と同じ。従来の `stale` / 未取得の札はそのまま） |
| グラフ・履歴の点（`/series` 由来） | **読み取り値**（今の文言のまま。変えない） |
| `/api/v1/health` がまだ届いていない | 確認中（今と同じ。実測とは言わない） |
| `?mock=` で模擬データを表示中 | 模擬（今と同じ） |

**なぜ鮮度で絞るか。** DB を使い回して `mock` → `hardware` に切り替えたとき、新しい合成で
無効にした入力（例: `nvml.enabled: false` で `expected_metrics` が空）のメトリクスは、
もう書かれないのに**古い模擬の行が `/latest` に残り続ける**（保持期間で消えるまで）。
種類は既に `hardware` なので、鮮度を見ないとその古い値に「実測」が付く。
届いている値にだけ札を付ければ、そうした行は従来どおり「読み取り値」に留まる。

- 見出し・凡例の札は「**いま届いている値**の出どころ」として書く。届いている内部テレメトリの
  値が1つも無ければ「読み取り値」（または health 未着なら「確認中」）にする
- **過去の出どころは画面に出さない。** グラフの点が実機の記録か模擬の記録かは表示しない。
  いまの値の札を過去の点へ広げない（§4 の案 G）
- 注記にもそう書く。例: 「回転数・PWM・CPU・GPU の**いま届いている値**は実機の読み取り値です。
  古い値とグラフの過去の値の出どころは表示しません。」
- **分からないときに「実測」と言わない。** 古いデーモン（キーを書かない版）の DB では
  `null` になり、今と同じ「読み取り値」に留まる
- 値ごとの `stale` / 未取得の表示（0046 / 0051）は変えない。出どころの札はその上に重ねる
- 札と注記の語は画面側（`airflow-status.js`）に持つ。`air.*` 用の `INGEST_KINDS` と同じ扱い
- **0046 §2.5 の表の「PWM」の行の札（`PWM（実測） 48%`）は、本記録の札に置き換わる。**
  実機で届いていれば「実測」、模擬なら「模擬」、種類が分からない・届いていない値は
  「読み取り値」。同じ行の他の内容（0〜100 % で保存する、Demand ではない）は変わらない
- **0051 §2.3 の「過去に保存された実測値を描く」も、「実測」とは言わない。** 収集を無効にした
  あとも過去の行をグラフに描くという規則は変えない。ただし模擬の adapter で書いた区間が
  あるため、グラフの点の出どころは主張しない（過去の出どころは表示しない）
- **0051 §2.1 の「内部テレメトリの『読み取り値』」も、CPU 使用率のいまの値については
  本記録の札に従う。** 0051 の「未計測 / 未取得」の判断（`cpu_utilization.measured`、
  `sys.telemetry_source.proc_stat` の状態から決める）は変えない。**値があるかどうか**は 0051、
  **その値の出どころ**は本記録で、別の軸として扱う
- 判定は純粋関数にし、既存の JS テスト（0044）で DB 無しに検証する。
  `stale` の値・`missing` の値・**`value: null` かつ `quality: suspect` の行**・
  種類が `null` の場合を必ず試す

### 2.6 受け入れる限界（仕組みで防がない）

次は**起こりうるが、表示の注記のために仕組みを足さない**。運用で実際に困ったときに、
新しい決定記録で扱う。

- **同じ DB へ2つのテレメトリデーモンが同時に書く構成**（片方が模擬）では、いまの種類が
  後から書いたほうになり、届いている値にも別のデーモンの種類の札が付きうる。
  この構成は同じメトリクスの主キーが重複して成り立たず、禁止も検査も足さない
- **`stale_after_ms` より短い間に種類を切り替えた**場合、切り替え前の値がまだ「届いている」と
  判定される時間帯がある（最大で `stale_after_ms`。既定 10 秒）
- **時計が戻った起動**では、種類の行が過去の時刻に入りうる。現在値しか読まないので表示は変わらない

止まったデーモンの種類が古い値に付く問題と、切り替え後に更新されなくなったメトリクスの
問題は、§2.5 の「届いている値にだけ札を付ける」で解消する。

### 2.7 範囲の外

- シリアルポートには触れない（取り込みデーモンだけが開く規則は変わらない）
- 制御（Supervisor / MPC / Guard / Critical Safety）は `sys.telemetry_kind` を読まない。
  これは表示のための情報で、制御の入力にしない
- **実装は本記録とは別の Issue / PR で行う**（#147 の後続）。デーモン・API・画面の変更と
  その試験をそこで入れる

## 3. Consequences

良くなること:

- 実機で動かしているとき、いま届いている回転数・PWM・CPU・GPU を「実測」と書ける
- 模擬の adapter で書いた DB を見ているときは「模擬」と出る
- 変更が小さい。`system_state` のキー1つ、`/api/v1/health` のフィールド1つ、画面の札だけで、
  スキーマ・既存 endpoint の応答・Server Health の契約は変わらない
- **記録の形を変えずに後から広げられる。** 種類は変化時だけ書くので、DB には
  「いつからどの種類か」が残る。過去の値にも出どころを出すと決めたときは、この行を
  区間として読む記録を新しく書けばよく、書き込み側の作り直しは要らない

悪くなること（と緩和策）:

- **過去の値の出どころが分からない。** グラフは今までどおり「読み取り値」のまま。
  → 画面の注記に「過去の値の出どころは表示しない」と明記する（§2.5）。
  必要になったら上の拡張余地で扱う
- **古い値・更新の止まったメトリクスには札が付かない**（「読み取り値」のまま）。
  → 出どころを誤って広げるより安全側に倒す。届いている値には付くので、通常運転では目的を満たす
- **残るずれ**（同時に2つのデーモンが書く、`stale_after_ms` 未満での切り替え）。
  → §2.6 に限界として書き、仕組みは足さない
- **`hardware` は偽の sysfs を指す設定でも名乗れる**（§1 の 3）。
  → 意味を「実 adapter が設定された読み取り口を読んでいる」に限定して明記する（§2.1）。
  偽の sysfs を使うのは試験だけで、試験は `build(adapters=...)` か adapter を直接使う
- **合成の起点の関数に引数が1つ増え、既定値を持たない**ため、`build(adapters=...)` を呼ぶ
  既存の試験はすべて `source_kind` の追記が要る。
  → 既定を持たせないことが目的（偽 adapter が黙って実機を名乗らない）なので受け入れる

## 4. 却下した代替案

- **Q. 種類の行を区間の履歴として読み、過去の値にも出どころを出す**
  （本記録の改訂案。2026-09-20 に所有者が却下）。
  `system_state` の行を `[t_i, t_{i+1})` の区間として読み、`GET /api/v1/telemetry-source` で
  返し、値の時刻（集計の点はバケット全体）で札を決める案。表示の注記のために、
  endpoint の追加・バケット幅の扱い・`/latest` への値ごとの時刻の追加・時計が戻った起動の
  `unverified` 印・書き手が1つであることへの依存と、縁のケースが次々に必要になった。
  **得るもの（過去の点の出どころ表示）に対して作るものが大きい。** いまの値の札だけで
  当面の目的（実機のときに「実測」と書く）は満たせる。記録の形は変えないので、
  必要になったときに新しい決定記録で足せる（§3）
- **G. いまの種類の札を、グラフ・履歴の点にも当てる。**
  DB を使い回して種類が切り替わると、模擬の履歴に「実測」が付く。過去には札を付けない
  （§2.5）ことで避ける
- **A. 画面が Server Health の稼働状態（`nvml: ok` など）を見て「実測」と書く。**
  稼働状態は「読めたか」であって「何を読んだか」ではない。偽 adapter でも `ok` になる
- **B. adapter ごとのキー（`sys.telemetry_kind.nvml` など）で記録する。**
  今は1つのデーモンの adapter がすべて同じ合成で組まれ、種類が混ざらない。画面の札も
  内部テレメトリ全体で1つ。画面側にメトリクス → adapter の対応表を持つ必要も生じ、
  設定（`internal-telemetry.yaml`）と二重管理になる。混ぜる合成が必要になったら改めて決める
- **C. adapter のクラスが種類を申告する（Protocol に `kind` を足す）。**
  `NvmlAdapter(api=...)` のように実クラスのまま中身を差し替えられるため、クラスでは
  判別できない（§1 の 3）
- **D. `internal-telemetry.yaml` に `source_kind: hardware` と書く。**
  設定は運用で変える値の置き場で、「実機かどうか」を人が申告すると、試験用の設定を
  流用したときに誤った値が残る。合成の起点のほうが事実に近い
- **E. `sys.telemetry_source.<adapter>` の値に種類を混ぜる（`ok:mock` など）。**
  0040 の `HealthSourceStatus` の解釈が壊れ、知らない値は `unavailable` に落ちる
- **F. readings の各行に出どころの列を足す。**
  行単位で最も正確だが、マイグレーションが要り、全行に同じ値が並ぶ。ロールアップの行にも
  列が要る。いまの値の札には要らない
- **L. 導入前の記録を、所有者の申告で遡って `hardware` にする。**
  根拠が人の記憶になり、「分からないときに実測と言わない」に反する

## 5. 未決事項

- **過去の値の出どころ表示**: 必要になったら、§2.3 の行を区間として読む記録を新しく書く
  （案 Q の内容を土台にできる）
- **`replay` の追加**: 内部テレメトリを再生する経路を作るときに、その記録で決める
- **Thermal dataset（0031）での利用**: 学習用データを `hardware` の区間に限るかどうかは、
  学習パイプラインの側で決める
- **Server Health への公開**: Workspace で出どころを見せる必要が出たら、0040 / 0042 の後続で決める
- **取り込み経路（`air.*`）の札**: `sys.ingest_source` も現在値1つで決めており、同じ限界がある
  （0046 §2.7）。扱うなら別の Issue
