...
**安全側へは常に動ける**（試験で確かめる）。さらに、降格の適用そのものは
authority の lock も disk も待たない（§2.6）。

**部品ごとの失敗は、その部品の理由で残す。** Registry を pin する context manager の
...
**適用は永続化を待たない。** 手順は「**先に in-memory の上限を下げ**、そのあとで journal へ
書く」である。`lower_stage()` は flock と fsync を待つので（別 process が昇格で lock を
握っていればその間ずっと待つ）、下げるのを書き込みのあとや例外の枝に置くと、
**待っている間ずっと前の stage で回る**（codex #4056992239）。例外の種類にも依存させない。

そのうえで、

- **書けたら** in-memory の上限を手放す。journal がその降格を表しているので二重に持たない。
  持ち続けると、記録に残った降格のあとで承認された昇格が process を作り直すまで
  効かなくなる（codex #4056968495）。下げ先は必ずその時点の上限以下なので、手放しても
  authority は上がらない
- **書けなかったら** in-memory の上限をそのまま持ち続ける。**読み直し（`reload()`）でも
  外れない。** 外れるなら、記録の無い降格を「読むだけ」で取り消せてしまう

書けなかったら元へ戻す実装にはしない。disk が一杯な間ほど高い authority で回り続ける。
