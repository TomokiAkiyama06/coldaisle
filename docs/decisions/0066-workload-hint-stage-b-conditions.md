...
- そこで、領域の中の tick でも**ヒント有りとヒント無しの両方を解き、(b) と同じ 2 条件
  ...
  - 2 回の solve は**1 つの tick の予算の中に束ねる**。順序は固定で、**先にヒント無し（基準）を解き**、
    その後に残った予算だけをヒント有りの solve へ渡す。合計が `tick_deadline_ms` を超えることはなく、
    後段の Guard / Safety と heartbeat を遅らせない
...
- **`contradicted` などの終端状態は制御デーモン自身が永続化し、再起動で復元しない。** 0064 §2.7 では矛盾は
  ...
  - **run の開始記録が永続化されるまで、その run はヒントを使わない。** 開始記録の書き込みに失敗した run は、
    ヒントを prior に使わず記録のみとして動く（`hint_prior_skipped: run_start_not_durable`）。開始記録が無いまま
    ヒントを使って終端状態の保存にも失敗すると、次の起動で「直前の run」が正常に見えてしまうため
