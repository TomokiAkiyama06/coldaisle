...
- **Supersedes**（いずれも限定的な置き換え。旧記録は FINAL のため本文を書き換えず、`Superseded by` の追記だけを行う）:
  - [0064](0064-workload-hint-entry-and-supervisor-prior.md) §2.8 の「単調性（片方向）」の検証方法、§2.6 の
    「起動時の取り込み」の条件と §2.6 の「`events.ts_ms` を行の順序に使う」こと（起動時の復元と稼働中の増分取り込みの両方）、
    §2.4 の「表・トリガ・索引は 0045 §2.5 のままでスキーマ変更は要らない」の記述、
    §2.5 の「`events` を読むのは制御デーモンで、読み取り専用の接続で読む」の記述のうち、制御デーモンが DB に
    一切書かないと読める部分（`events` は引き続き読み取り専用の接続で読む。本記録の §2.2 で、制御デーモン自身の表への
    書き込み用の別の接続が加わる）、
...
- **許容誤差そのものがゲートを無効にしないよう制約する。** `eps` / `eps_c` は有限・非負で、
  ...
  **上限そのものにも同じ制約をかける。** `max_eps` / `max_eps_c` は有限・非負（YAML の `.inf` / `.nan`・負値・
  欠落は不正）で、`max_eps < 1.0` とする。`max_eps_c` にも有限の天井を置き、その値は §5 の未決 #1 と同じく実測で決めて
  `config/safety.yaml` のスキーマに持たせる（コードに定数を置かない）。上限が不正なら、`eps` / `eps_c` の値に関わらず
  Stage B を無効のまま起動する（上限の検査を通らない限り、`eps` の比較を始めない）。
...
- (b) は起動時に走らせない（重い・非決定性の温床になる）。代わりに、試験が通ったときの
  **試験の全入力と実装の識別子を束ねたハッシュ**を成果物として記録し、Stage B を有効にする設定は
  ...
  - **Thermal Model の推論の実装の識別子**（`predict` と前処理・特徴量の組み立てを含むモジュールの
    ...
    上がらないため、版ではなく内容で束ねる
  - **探索のコストに入る他のモデルと条件**: `LearnedMpcController.conditions()` が「結果に効くすべて」として返す値の全体
    （Acoustic Model の metadata・Air Balance Model の metadata・`BalanceBand`・Confidence Profile の digest・
  ...
  - **実装は列挙ではなくソースツリー全体で束ねる。** 上の各「モジュールの内容ハッシュ」は説明のための例であり、
    実際に束ねるのは `src/coldaisle/` 配下の全ソース（パスの昇順に、各ファイルを「相対パスの UTF-8 バイト長（8 バイト big-endian）・相対パス・内容のバイト長（同）・内容」と長さ付きで符号化して連結した列の SHA-256。区切りの無い連結は、パスと内容の境界をずらした別のツリーと同じ列を作れるため使わない）と
  ...
  - **(b) の試験を行う実装も `src/coldaisle/` に置く。** 格子の生成・二重解法・全 offset の比較・合否の判定・
    成果物（ハッシュ）の生成は、`src/coldaisle/` 配下の production module（認証ランナー）に置き、`tests/` や
    リリーススクリプトはそれを呼ぶだけにする。判定や格子を `tests/` 側で書き換えると、上の digest に含まれず、
    検査の軸や offset を減らしたまま同じハッシュを作れてしまうため。成果物には認証ランナーが自分で計算した
    digest を書き、`tests/` 側から値を渡さない
  - **実行環境も束ねる。** 同じ `src/coldaisle/` とロックファイルでも、Python の処理系と版・OS とプラットフォーム・
    CPU アーキテクチャ・実際に入った wheel（数値計算ライブラリのバイナリなど）が違えば、予測や探索の結果は変わりうる。
    そこで成果物に**実行環境の指紋**（処理系の実装と版、`sysconfig` のプラットフォーム、`platform.machine()`、
    インストール済みの配布物の名前・版・`RECORD` のハッシュから作る digest）を含め、起動時に同じ手順で計算した指紋と
    一致しなければ Stage B を無効のまま（ヒントは記録のみ）で起動する
    - したがって (b) は、本番と同じ実行環境（本番のホスト、または指紋が一致する環境）で走らせる必要がある。CI の
      汎用ランナーで作った成果物は、本番の指紋と一致しない限り Stage B を有効にしない
...
- **稼働中のヒントの期限も `CLOCK_BOOTTIME` で数える。** 0064 §2.6 は、ヒントの `observed_monotonic_ms` を
  `ControlStateSnapshot.monotonic_ms`（`CLOCK_MONOTONIC`）と同じ時計で刻むと定めるが、`CLOCK_MONOTONIC` は suspend 中に
  進まないので、デーモンを止めずにホストが suspend すると期限切れのヒントが再開後も残り時間ぶん有効になる。そこで
  ヒントの観測時刻・期限（TTL）・矛盾の保持時間は、稼働中も**`CLOCK_BOOTTIME`** で刻んで数える
  - 時計は `coldaisle.clock` の抽象に `CLOCK_BOOTTIME` の読みを足して渡し（`WallClock` は実時計、`SimulatedClock` は
    シナリオ時間）、Mock / Replay で再現できるようにする。制御ループの他の判定（tick の deadline・stale・hold・stall）が
    使う `ControlStateSnapshot.monotonic_ms` は変えない（0028 の契約のまま）
  - 起動時の取り込み（上）と稼働中で同じ時計を使うので、起動時に復元した残り時間をそのまま稼働中の期限へつなげる
  - **期限の起点は、読んだ tick ではなく行に記録された `boottime_ns`** とする（稼働中の初回取り込みでも同じ）。
    行が届いてから読まれるまでの間に suspend をはさむと、読んだ時刻を起点にすれば未読の間の経過が差し引かれず、
    期限切れのヒントに満額の期限を与えてしまうため。稼働中に初めて読む行にも起動時と同じ条件を当てはめる:
...
- **行の順序は壁時計で決めない（起動時の復元も、稼働中の増分取り込みも）。** 0064 §2.6 は `events.ts_ms` を行の順序に使うが、
  ...
  - 稼働中の増分取り込みも同じく `events.id` の昇順で行い、読み進めた位置は最後に処理した `events.id`
    （時刻ではない）で持つ。稼働中に壁時計が戻った後に追記された終了・置き換えの行も、`ts_ms` が開始の行より
    小さくても追記順に処理されるので、読み落としたり開始より前に当てはめたりしない。`ts_ms` の値は
    記録として残すだけで、順序にも読み進めた位置にも使わない
- **`contradicted` などの終端状態は制御デーモン自身が永続化し、再起動で復元しない。** 0064 §2.7 では矛盾は
  ...
  - **ヒントの状態を読めないときは、空と扱わずヒントを使わない。** 終端状態の表・run の記録・取り込みの境界・
    `events` のどれかを正常に照会できない場合（ロック中・破損・migration の不整合・列の欠落など）は、「該当なし」とは
    扱わず、その照会が成功するまでヒントを prior に使わず記録のみとして動く（`hint_prior_skipped: hint_state_unreadable`）。
    ヒントは任意の機能なので、この失敗で制御デーモンを止めない。制御はヒント無しの既存経路（Supervisor / MPC /
    Guard / Safety）で続ける。稼働中に照会が失敗した場合も、その時点で使っていたヒントを外して同じく退避する
  ...
  - **run の順序は時計に依存しない追記順の番号で決める。** run の開始記録は、`events.id` と同じく追記だけで増える
    番号（`run_seq`。SQLite の `INTEGER PRIMARY KEY AUTOINCREMENT` のように、再利用も巻き戻りもしないもの）を持ち、
    正常終了・「欠けあり」の記録はその `run_seq` で開始記録に対応付ける。「直前の run」は `run_seq` が最大の
    開始記録であり、開始時刻（壁時計・boot id・`CLOCK_BOOTTIME`）では並べない。壁時計の巻き戻りや再起動で
    古い正常終了の run を「直前」と取り違えないため
    - 直前の run の終了記録が無い、2 件以上ある、対応する開始記録が無い、状態が既知の値（正常 / 欠けあり）以外、
      のいずれかなら異常終了と同じに扱う（未知の lifecycle の状態を正常として扱わない）
  ...
  - デーモンが止まっている間に届いたヒントは、直前の run が正常に終わっていれば、終端状態が書かれている
    はずもないので、上の条件で取り込める
  - **`events` は読み取り専用のまま。** `events` は 0064 §2.5 のとおり読み取り専用の接続で読み、制御デーモンは
    `events` に書かない。終端状態の表・run の記録・取り込みの境界は、制御デーモンが**自分の表にだけ書く別の接続**で書く
    （書き込み入口は 0045 のソケットだけ、という `events` の契約は変えない）。DB を開くのは合成の起点である制御デーモンで、
    `coldaisle.control` は引き続き `coldaisle.store.db` を import しない
  - 表と run の記録の形（`run_seq` 以外の列・migration）は、0066 の実装 Issue で定める
