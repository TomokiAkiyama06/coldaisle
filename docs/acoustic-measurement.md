...
- 普段の回答は 3 Zone が同時に動いているので、1つの回答からはどの Zone がうるさいか分からない。
  ...
  その Zone だけが高い回答と、その Zone だけが低い回答がそれぞれ十分あるときに限る（0067 §2.4）。
  足りない Zone の曲線は変えない
- 次の回答は分けて見る
  - CPU / GPU が高負荷のとき
  - GPU 本体の Fan（`gpu.0.fan_speed`）が回っているとき
  - 回答の直前の区間のどこかで decision trace の tick が途切れていたとき（tick の間隔が記録された周期の許容倍数を超えた）、またはその区間で Demand が変わっていたとき（状態が分からないので使わない）
