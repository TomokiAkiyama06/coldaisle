...
def test_calibration_brings_the_spread_within_the_target(store, catalog, tmp_path, rules):
    ...
    corrected = [item.mean_c + calibrated.offsets_c[item.channel] for item in after]
...
def test_the_offsets_cancel_the_individual_bias(store, catalog):
    ...
    for metric, offset in BIAS.items():
        ...
        assert result.offsets_c[channel] == pytest.approx(-offset, abs=0.02)
...
def test_the_reference_is_the_mean_of_all(store, catalog):
    ...
    assert sum(result.offsets_c.values()) == pytest.approx(0.0, abs=1e-9)


# ---------------------------------------------------------------- 受け付けない条件


def test_a_large_spread_is_refused(store, catalog):
    """**同じ空気に置かれていないなら較正しない。**
    ...
    **以降その勾配が見えなくなる。**
    ...
    assert result.spread_c > MAX_SPREAD_C
    assert result.usable is False
    assert any("同じ空気" in problem for problem in check(result))


def test_a_spread_within_the_limit_is_accepted(store, catalog):
    result = result_for(store, catalog)
    assert result.usable is True
    assert check(result) == []


def test_too_few_samples_are_refused(store, catalog):
    """**少ない標本で較正しない。** 雑音をオフセットとして焼き付ける。"""
    result = result_for(store, catalog, count=10)
    assert any("測定が少ない" in problem for problem in check(result))


def test_a_missing_channel_is_refused(store, catalog):
    ...
    assert any("測定が無い" in problem for problem in check(result))


def test_humidity_is_not_calibrated(store, catalog):
    """**%RH に ℃ のオフセットを足さない**（要件 §5.1）。"""
    result = result_for(store, catalog)
    assert "room_humidity" not in result.offsets_c
...
def test_the_record_carries_its_evidence(store, catalog, tmp_path):
    ...
    assert written.samples["room_temp"] >= MIN_SAMPLES
...
def test_an_uncalibrated_file_is_treated_as_expired():
    ...
    assert Calibration().is_expired(NOW_MS) is True
...
def test_a_fresh_calibration_is_not_expired():
    ...
    assert calibration.is_expired(NOW_MS) is False
...
def test_an_old_calibration_expires():
    ...
    assert Calibration(calibrated_at=at.isoformat()).is_expired(NOW_MS) is True
