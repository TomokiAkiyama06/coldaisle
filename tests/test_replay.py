...
フィクスチャは実ファイルと同じ体裁の合成データ。**実機の記録はリポジトリへ
持ち込まない**（#41）。
"""
...
DAY_24 = FIXTURES / "sensors_2026-08-24.csv"
...
def source(path: Path = DAY_24, **kwargs) -> ReplaySource:
...
def test_rows_become_samples():
    produced = samples(source())
...
def test_hello_comes_first_and_estimates_the_interval():
...
def test_column_names_are_tolerated():
    """列名の揺れに耐える（#7）。知らない列は捨てて続ける。"""
    produced = samples(source(FIXTURES / "replay_odd_columns.csv"))
...
def test_a_directory_is_read_in_date_order():
    produced = samples(source(FIXTURES))
...
def test_host_time_comes_from_the_csv():
...
def test_timezone_is_taken_from_the_caller():
...
def test_synthesised_sequence_is_continuous():
...
def test_every_mode_produces_the_same_samples():
    """速度は待ち時間にだけ効く。保存される内容は同じ。"""
    modes = [
        [sample.model_dump() for sample in samples(source())],
        [sample.model_dump() for sample in samples(source(speed=60.0))],
        [sample.model_dump() for sample in samples(source(bulk=True))],
    ]
...
def test_replayed_data_lands_in_the_store_with_its_original_time(tmp_path, rules):
    ...
    replay = source(FIXTURES, bulk=True)
...
def test_daemon_builds_a_replay_source(tmp_path):
