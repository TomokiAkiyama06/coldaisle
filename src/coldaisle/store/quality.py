...
**「前サンプルとの差が大きい場合だけ疑う」という条件を足さないこと。**
足すと、最初の1サンプルだけ差が大きくて `suspect`、以降は差がゼロなので `ok` になり、
**故障が継続している間だけ正常扱いになる。** 85.00 への張り付きこそが典型的な
壊れ方なので、条件を足すと狙いと逆の挙動になる。

無条件で疑うことによる誤検出は小さい。ここで測るのは空気温度であり、
この位置で 85.00 ちょうどが実測される状況は事実上ない。仮に本物でも
`suspect` が5サンプル続けば `SENSOR_MISSING`（FR-402）が発火するため、
握り潰されることはない。
"""
