...
OBSERVED_PROBES_KEY = "sys.observed_probes"
"""**いま繋がっている ROM**（`system_state`。#14 / FR-403）。

記録（`device_sensors`）と食い違っているあいだだけ入る。値は
`{"チャネル": "ROM"}` の JSON。

記録の側は人が較正をやり直すまで動かない（決定記録 0012 §2.6）ので、
**このキーが「差し替えたあとの現物」を指す唯一の手がかり**になる。
これが無いと、人は「何に差し替わったのか」を知る方法が無い。
"""
