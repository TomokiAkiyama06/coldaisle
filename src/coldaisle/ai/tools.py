...
GUIDANCE = (
    "これらのツールは読み取り専用です。**このツール群と LLM は**、"
    "ファン制御・電源操作・シャットダウンを一切行えません（AGENTS.md ルール1）。"
    "Fan 制御を行う場合も、LLM とは切り離された別系統が決定論的な安全装置を通して行い、"
    "ここからは変更できません（決定記録 0027）。"
...
class ToolRegistry:
    ...
    def _tool_query_series(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        AGENTS.md のルール8（生の時系列を LLM のプロンプトに直接入れない）は
